...
const CommentsSection = ({ postId, isAdmin }) => {
  ...
  if (isError) {
    return (
      <div role="alert" className="alert alert-error">
        <span>
          Sorry, something went wrong. Cannot load comments at the moment.
        </span>
      </div>
    );
  }
        ...
        {comments.length === 0 ? (
          ...
          comments.map((comment) => (
            <div key={comment.id} className="chat chat-start">
              ...
              <div
                className={`chat-bubble text-sm ${userId === comment.authorId ? 'chat-bubble-primary' : 'chat-bubble-neutral'}`}
              >
                {comment.content}
              </div>
